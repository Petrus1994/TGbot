...
async def submit_profiling_answer(goal_id: str, answer: str) -> dict:
    ...
    with engine.begin() as connection:
        ...
        try:
            ...
            print(f"❌ profiling_answer_evaluation_failed: {e}")
        ...
        try:
            next_step_raw = await dynamic_profiling_service.select_next_step(
                goal_title=goal["title"],
                goal_description=goal.get("description"),
                questions=questions,
                answers=answers,
            ...
            print(f"❌ profiling_next_step_selection_failed: {e}")
            raise HTTPException(status_code=500, detail="profiling_next_step_selection_failed")

        next_step = _sanitize_next_step(next_step_raw)
        ...
        if not next_question:
            next_question = _pick_next_unanswered_question(
                questions=questions,
                answers=answers,
                asked_question_keys=asked_question_keys,
                skipped_question_keys=skipped_question_keys,
